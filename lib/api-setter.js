...
function pagination(options){
    ...
    return function (req, res, next) {
        ...
        options = _.merge(options, req.query);

        if (options.export) {
            options.limit = 0;
        }

        req.api.pagination.limit = parseInt(options.limit);
        req.api.pagination.page = parseInt(options.page);
        req.api.pagination.skip = (options.page > 0 ? ((options.page - 1) * options.limit) : 0);
